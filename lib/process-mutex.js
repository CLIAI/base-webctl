...
export function createProcessMutex(C, opts = {}) {
  ...
  // legacyHomeOnly is REQUIRED, not cosmetic: honouring $XDG_CACHE_HOME would
  // move the lock directory on upgrade, and a lock directory that moves is a
  // lock that stops working DURING the rollout — one process on the old base
  // and one on the new take locks in different directories and BOTH believe
  // they hold it. That is the mutual-exclusion failure this module exists to
  // prevent, caused by upgrading it.
  const lockBaseDir = opts.lockBaseDir ||
    createStoragePaths(C, { legacyHomeOnly: true }).locksDir;
