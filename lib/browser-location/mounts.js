...
export function createMounts(C, opts = {}) {
   ...
   *            slugIsDefault: boolean, warning: string|null}}
   */
  function describeProfileResolution(slug, userDataDir) {
    ...
    if (!slugIsDefault && isDefaultProfile) {
      warning = `⛔ slug '${slug}' looks like a throwaway, but the profile resolves to `
      ...
      warning = `slug '${slug}' is not 'default', but userDataDir is set explicitly, so `
