 ...
 */
/**
 * ⭐ THE XPRA CONTAINER ENV CONTRACT, AS DATA.
 *
 * base SETS these values; a consumer-owned ENTRYPOINT consumes them. Until
 * 2026-09-22 that contract existed only in this file's comments, so it was
 * enforced in N consumer-written shell scripts and re-derived by copying — and
 * the only thing that could detect a mismatch was starting a container. A lane
 * can be green on its whole unit suite and dead on `docker up`.
 *
 * ⛔ `forbidden` IS A REAL LIST, NEVER THE COMPLEMENT OF `required`. "Not
 * required" and "must not be set" are different claims, and only the second
 * catches an entrypoint still consuming a variable base STOPPED setting —
 * which is the live failure: an entrypoint that requires XPRA_HTML5_BIND
 * hard-fails at startup against any base at or after the v0.6.0 port collapse.
 *
 * ⚠ FLAT, PLUS `since` — deliberately not a version-keyed map. A map is a
 * second source of truth maintained by hand, wrong the moment someone adds a
 * key without changing behaviour, and it invites a consumer to LOOK UP ITS OWN
 * PIN rather than observe the driver. `since` is for the human reading a
 * failure: it turns "XPRA_HTML5_BIND is required but not passed" into "your
 * entrypoint predates v0.6.0".
 *
 * ⚠ AND IT IS NOT THE GROUND TRUTH. A consumer asserts against CAPTURED
 * `runDetached` args — what the driver actually passes — and uses this only for
 * the message and the forbidden set. Verifying base's behaviour by grepping
 * base's source returns the NEGATION here: a grep for XPRA_HTML5_BIND in this
 * file matches the comments saying it is deliberately not set.
 *
 * (Shape specified by fetlife-webctl, which built the reference consumer-side
 * assertion before this existed.)
 */
export const XPRA_CONTAINER_ENV_CONTRACT = Object.freeze({
  required:  Object.freeze(['XPRA_TCP_BIND']),
  forbidden: Object.freeze(['XPRA_HTML5_BIND']),
  htmlFlag:  '--html=on',
  since:     'v0.6.0',
});

/**
 * ⭐ CONTAINER LIFECYCLE, AS DATA — and the omission is a DECISION.
 *
 * base starts containers with `docker run -d` and **no `--restart` policy**, so
 * ⛔ **nothing this family starts survives a reboot.** Until 2026-09-22 that was
 * true, unstated, and invisible from any consumer's code.
 *
 * It cost two weeks: a lane's containers were EXITED from a reboot while it
 * reported "containers up ~33h" to three audiences, because it kept quoting a
 * reading taken before the boot. ⚠ Its *design* was sound — the failure was a
 * status claim that aged into a lie, where nothing breaks and the claim simply
 * stops being true.
 *
 * ⭐ RULED: NO RESTART POLICY, DELIBERATELY. Not an oversight to be tidied up:
 *
 *   1. Several lanes hold sessions authenticated as a real person, and one
 *      reads other people's personal data. A browser carrying such a session
 *      resurrecting UNATTENDED at boot — holding a profile lock and a published
 *      CDP port, with nobody present — is materially worse than one that is
 *      simply down.
 *   2. Bring-up here is an explicit act (`ensureRunning`), and the profile lock
 *      records the owning container on the assumption a runner is attached. A
 *      container revived by dockerd has no runner, so the lock would read as
 *      held by a live owner that does not exist.
 *   3. Turning 14 lanes' authenticated browsers into boot services is a
 *      security-posture change, and not one base makes silently on their behalf.
 *
 * ⇒ This is recorded so the next reader finds a DECISION rather than an
 * apparent gap. An unstated omission gets "fixed", and this one gets fixed by
 * adding the flag.
 *
 * ⚠ The operational consequence belongs in every lane's status surface, not in
 * prose: after a reboot a lane is `configured: true` with its containers gone,
 * and a reachability probe is the only thing that notices.
 *
 * (Measured and escalated by fetlife-webctl, which checked base's `runDetached`
 * before assuming the affected lane had misconfigured itself.)
 */
export const CONTAINER_LIFECYCLE_CONTRACT = Object.freeze({
  restartPolicy: null,
  survivesReboot: false,
  startedWith: 'docker run -d (no --restart)',
  // ⛔ THE PAIR HAS AN ORDER, AND IT IS THE DECISIVE REASON — stronger than the
  // authenticated-browser argument below, because it is a CORRECTNESS one.
  //
  // chromium runs with `--network container:<xpra>`: its network namespace IS
  // the xpra container, and it needs xpra's Xvfb socket on the shared volume.
  // ⇒ xpra must be up FIRST. Docker restart policies have **no ordering
  // primitive** — no depends_on, no health gate — so `--restart` CANNOT EXPRESS
  // this dependency. A policy on both containers reproduces, on every boot, the
  // failure measured on 2026-09-23 when a human was told to `docker start` the
  // pair:
  //
  //     chromium races xpra's X server -> exits 1, "Missing X server or $DISPLAY"
  //     `docker ps` shows a RUNNING container and the CDP port answers nothing
  //     ⇒ the reader concludes THE TOOL IS BROKEN rather than the instruction
  //
  // ⇒ So recovery is not a convenience question. Anything that brings the pair
  // back WITHOUT the driver gets the order wrong, and gets it wrong in the
  // shape that looks like a product defect.
  pairOrder: Object.freeze(['xpra', 'chromium']),
  recovery: 'driver',
  recoveryNote:
    'Come back through ensureRunning(). ⛔ NOT `docker start`: it starts '
    + 'containers without sequencing, so chromium races xpra\'s X server and '
    + 'exits while `docker ps` reports it running.',
  since: 'v0.1.0',
});

/**
 * ⭐ WHAT `shutdown()` LEAVES, AS DATA.
 *
 * Same class of fact as the env contract: base-owned, restated in every
 * consumer's user-facing teardown message, and checkable by nobody. `shutdown`
 * is the one driver verb whose NAME implies more than it does.
 *
 * ⛔ `removes` IS EMPTY, AND THAT IS THE POINT. A consumer that prints
 * "containers removed" after calling this is wrong — measured, by a lane that
 * published exactly that sentence while writing it to carry a warning about
 * teardown verbs overstating.
 */
export const TEARDOWN_CONTRACT = Object.freeze({
  verb: 'shutdown',
  stops: Object.freeze(['chromium container', 'xpra container']),
  removes: Object.freeze([]),
  // ⚠ KEYED, not prose. A consumer asserts that every kept artifact is named in
  // its user-facing teardown message — and substring-matching prose means
  // rewording an entry silently WEAKENS that check without failing anything.
  // The key is the contract; the text is for humans. (Asked for by
  // fetlife-webctl, which is the lane actually running that assertion.)
  keeps: Object.freeze([
    Object.freeze({ key: 'containers', text: 'containers (STOPPED, not removed)' }),
    Object.freeze({ key: 'volumes',    text: 'volumes' }),
    Object.freeze({ key: 'network',    text: 'network' }),
    Object.freeze({ key: 'profileDir', text: 'profile directory' }),
  ]),
  consequences: Object.freeze([
    'stopped containers still hold the volume, so `docker volume rm` refuses '
    + 'until they are removed by name',
  ]),
  since: 'v0.1.0',
});
...
export function createChromiumDockerXpra(C, opts = {}) {
  ...
  function createDriver(cfg) {
    ...
    // ⛔ PURE AT CONSTRUCTION. This used to call resolveChromiumProfile(), which
    // `mkdir -p`s — so merely BUILDING a driver created a profile directory,
    // before any bring-up and whether or not one ever happened. Every consumer
    // that constructs a driver to read inspect() — which is exactly what a
    // `gui status` does — littered a profile dir per slug, and base's own
    // registry suite wrote into the real ~/.cache on every run.
    //
    // ⚠ The generalisation is the reason this is here rather than in the test:
    // a consumer fixed the eager mkdir in ITS OWN resolver and recorded the fix
    // as complete, while THIS call site went on creating directories.
    // ⇒ FIXING AN EAGER MKDIR AT ONE CALL SITE SAYS NOTHING ABOUT THE OTHERS.
    //
    // The directory is created in ensureRunning(), before its first use.
    //
    // ⚠ WHAT THIS DOES NOT BUY. "The mkdir moved" and "consumers no longer need
    // a scratch userDataDir" are DIFFERENT CLAIMS, and the second does not
    // follow for anyone CAPTURING driver behaviour: a capture test drives
    // ensureRunning(), because that is the only path producing the runDetached
    // args it asserts on — so the directory still appears. Confirmed by a lane
    // that removed its workaround on the strength of the first claim and
    // watched profiles/<slug> come straight back.
    // ⇒ A capture test still needs a throwaway userDataDir (or a stubbed
    // resolver) or it seeds the real cache. The fix buys the OTHER case: a
    // read-only caller — `gui status` constructing a driver to read inspect() —
    // no longer litters a profile dir per slug, which is the use that mattered.
    const profilePath = mounts.profilePathFor(slug, cfg.userDataDir || null);
    ...
    async function ensureRunning() {
      ...
      // Create the profile dir HERE — the first point that actually needs it.
      // Construction stays pure (see profilePathFor above).
      mounts.resolveChromiumProfile(slug, cfg.userDataDir || null);
...
${indent(r.stdout || r.stderr, '    ')}`,
    ...
    async function shutdown() {
      ...
      //
      // ⛔ THIS STOPS. IT DOES NOT REMOVE. What survives a shutdown():
      //
      //     containers   STOPPED, not removed  <- omitted from this list until
      //                                           2026-09-22, and it is the one
      //                                           with a downstream consequence
      //     volumes      kept
      //     network      kept
      //     profile dir  kept
      //
      // ⚠ EXITED CONTAINERS STILL HOLD THE VOLUME, so a caller that stops and
      // then runs `docker volume rm` gets a refusal it has no way to predict
      // from this verb's name. Measured by fetlife-webctl on a live teardown.
      //
      // ⭐ The earlier comment listed three kept things and silently omitted the
      // fourth. A list that READS AS EXHAUSTIVE licenses the inference that
      // anything absent is removed — which is worse than no list, and is the
      // inference that lane drew and then published. `shutdown` is the one verb
      // here whose NAME implies more than it does, so the enumeration has to be
    ...
    function inspect() {
      ...
      const srcOf = (k) => {
        const ps = /** @type {any} */ (cfg).portSources;
        return (ps && typeof ps === 'object' && ps[k] != null) ? String(ps[k]) : null;
      };
      /** @type {any} */
      const out = {
        ...
        // ⭐ EACH PORT WITH WHERE IT CAME FROM. The numbers alone cannot answer
        // "why is it that number", which is the first question anyone debugging
        // a collision asks — and the derived-vs-configured distinction is the
        // one that cost this family weeks when a DERIVED html5 port was
        // published, reserved and advertised while answering nothing.
        //
        // ⚠ `source` is absent, not guessed, when the driver was built from a
        // cfg that predates portSources — an older consumer passing its own
        // object. Absent reads as "unknown"; a fabricated 'derived' would read
        // as a measurement.
        ports: {
          cdp: { value: port, source: srcOf('cdp') },
          'xpra-tcp': { value: xpraTcpPort, source: srcOf('xpra-tcp') },
          'xpra-html5': { value: xpraHtml5Port, source: srcOf('xpra-html5') },
        },
      };
      if (uploadDirs) {
