...
export function createLogRotation(C) {
   ...
   *
   * ⛔ RULED 2026-09-05: `keep = 0` IS ACCEPTED, and this is not an oversight.
   * linkedin-webctl proposed refusing it outright, since budget 0 deletes every
   * file we own. Refused, on three grounds, the first measured against this
   * module:
   *
   *   1. After validation the ONLY inputs that can still reach budget 0 are a
   *      literal `0` and the string `"0"`. `''`, `null`, `undefined`, `NaN`,
   *      `false`, `[]` are all refused by name. The validator closed the
   *      ACCIDENTAL-zero hole; rejecting 0 re-closes a door that is already
   *      locked, and pays for it below.
   *   2. `keep = 0` is precisely what `maxFiles: 1` MEANS under the ruled
   *      call-site idiom (`maxFiles - 1`). "Keep at most one log" is an ordinary
   *      thing to configure, and refusing it makes that unrepresentable — with a
   *      confusing error, for a user who did nothing wrong.
   *   3. A policy floor belongs to the CALLER or to `C`, never to the module.
   *      The caller knows whether it is about to open a file; the module does
   *      not — the same reasoning that put the `-1` at the call site. A
   *      module-wide refusal would bind every consumer to one consumer's policy,
   *      which is the "ship one repo's accident to the family" failure this
   *      module exists to avoid.
   *
   * ⇒ A consumer that never wants zero declares a minimum in `C` or returns
   * early in its own rotate(), as both current consumers already do.
   *
