...
/**
 * ⭐ THE COMPLETE SEVERITY VOCABULARY for `describeProfileResolution().warning`.
 *
 * Exported so a consumer can assert it knows every value, and **refuse an
 * unknown one** rather than assume it is benign. If base adds a severity, that
 * assertion fails loudly at the consumer instead of a new dangerous state being
 * silently allowed.
 *
 *   danger  ⛔ refuse the bring-up. Today: a slug claiming isolation that
 *              resolves to the DEFAULT — often authenticated — profile.
 *   notice  proceed; the configuration is fine but not what the caller may
 *              have assumed.
 *
 * ⚠ base assigns severity and does NOT refuse. A lane deliberately sharing one
 * profile is entitled to, so the policy stays with the consumer — but the
 * CLASSIFICATION belongs here, or every lane hardcodes its own and they drift.
 *
 * ⛔ PUBLISHING THIS LIST CLOSES *DRIFT* AND OPENS *COVERAGE*. A consumer that
 * checks only `PROFILE_WARNING_SEVERITIES.includes(sev)` is VACUOUS against the
 * case that matters: a severity base adds to this list AND emits passes that
 * check and falls straight through the consumer's unchanged branch — allowed,
 * silently. **Published is not the same as understood.**
 *
 * ⇒ A consumer needs TWO checks, deliberately not one:
 *
 *     drift     emitted severity ∈ THIS list        base disagreeing with itself
 *     coverage  emitted severity ∈ YOUR handling    base growing past you
 *
 * ⇒ And the coverage check belongs in a SUITE, not only at bring-up: a guard
 * that refuses at bring-up is the right backstop and it fires at the worst
 * moment, when someone is trying to start a container. Assert this list is a
 * subset of your classification in a test, so the vocabulary growing is a red
 * suite on an ordinary day. ⚠ Assert it BOTH ways — a value you classify that
 * base has DROPPED is a stale entry nobody would otherwise notice.
 *
 * (Worked out by fetlife-webctl, the first lane to consume this, after base
 * asked whether its unknown-value refusal survived base publishing the list.)
 */
export const PROFILE_WARNING_SEVERITIES = Object.freeze(['danger', 'notice']);
...
export function createMounts(C, opts = {}) {
   ...
   */
  function cacheRoot() {
    // CONSOLIDATED onto the v59v resolver (was a local $HOME derivation here).
    // `legacyHomeOnly` keeps the path BYTE-IDENTICAL for existing installs:
    // honouring $XDG_CACHE_HOME here would relocate profiles on upgrade, and a
    // relocated profile is an EMPTY profile — a silently logged-out browser,
    // with the real session still sitting at the old path. Moving these is a
    // deliberate announced migration, not a side effect of consolidation.
    //
    // One deliberate behaviour change: $HOME unset now THROWS instead of
    // falling back to '/tmp'. The two sites being consolidated disagreed here
    // anyway ('/tmp' vs os.tmpdir()), so consolidation had to pick one, and
    // writing profiles and cookies somewhere world-readable is not the one.
    return createStoragePaths(C, { legacyHomeOnly: true }).cacheRoot;
  }
  ...
  function resolveChromiumProfile(slug, userDataDir) {
    // ⛔ ONE path computation, shared with the pure resolver. These must never
    // compute the path independently: two functions deriving the same value is
    // the coincident-fields failure, and it bites the moment a caller stubs one
    // of them — which happened within minutes of `profilePathFor` existing.
    const p = profilePathFor(slug, userDataDir);
    fs.mkdirSync(p, { recursive: true });
    return p;
  }

  /**
   * ⭐ PURE profile-path resolution — the same answer as
   * `resolveChromiumProfile()` WITHOUT the `mkdirSync`.
   *
   * ⛔ A QUERY WITH A FILESYSTEM SIDE EFFECT IS NOT A QUERY. Asking "which
   * profile would this slug open?" must not create it — otherwise a read-only
   * status command manufactures empty profile dirs, and a guard written to
   * avoid touching an authenticated profile reaches toward that profile's path
   * in order to answer. (Both observed: the second in a consumer's first cut of
   ...
   * @returns {string}
   */
  function profilePathFor(slug, userDataDir) {
    return userDataDir ? expandHomePath(userDataDir) : profileDir(slug);
  }

  /**
   * ⭐ Does moving the slug actually move the profile? Pure; creates nothing.
   *
   * ⛔ THE FOOTGUN THIS ANSWERS: when `userDataDir` is set, the slug is ignored
   * entirely, so a "throwaway" slug renames the containers and still mounts the
   * configured — often authenticated — profile. A lane told "test on a
   * throwaway slug, never default" followed that instruction and was pointed at
   * the authenticated profile anyway; only the profile LOCK stopped it, and a
   * lock fires AFTER the attempt.
   *
   * base reports; the CONSUMER decides whether it is an error. Two containers
   * on one profile is a legitimate thing to want, so base must not refuse for
   * everyone — but "you asked for isolation and will not get it" is nameable,
   * and naming it before docker is touched is the whole value.
   *
   * @param {string} slug
   * @param {string|null} [userDataDir]
   * @returns {{path: string, source: 'explicit-userDataDir'|'derived-from-slug',
   *            isolatedBySlug: boolean, isDefaultProfile: boolean,
   *            slugIsDefault: boolean,
   *            warning: {kind: string, severity: string, text: string}|null}}
   */
  function describeProfileResolution(slug, userDataDir) {
    const explicit = !!userDataDir;
    const slugIsDefault = !slug || slug === 'default';
    const isolatedBySlug = !explicit;
    const resolved = profilePathFor(slug, userDataDir || null);

    // ⭐ THE SAFETY QUESTION, ANSWERED SEPARATELY. `isolatedBySlug` answers
    // "did the SLUG do the isolating?"; a bring-up guard asks "will this touch
    // the DEFAULT profile?" Those diverge EXACTLY in the case that matters:
    //
    //   slug 'qa' + userDataDir '/tmp/isolated'        isolatedBySlug=false  SAFE
    //   slug 'qa' + userDataDir <the default profile>  isolatedBySlug=false  ⛔
    //
    // Both are "not isolated by the slug"; one is a perfectly good isolated
    // bring-up, the other is a container named as a throwaway mounting the
    // authenticated profile. Shipped v0.11.0 with only the first field, so a
    // consumer branching on it — the obvious use — got the same answer for
    // both. Same shape as a field named for the fact while computing a proxy
    // that usually travels with it. (Found by fetlife-webctl consuming it.)
    const isDefaultProfile = resolved === profileDir('default');

    // ⛔ THE WARNING CARRIES A STABLE `kind`, NOT JUST PROSE. It fires for two
    // materially different situations, and `warning !== null` is true for both:
    //
    //   default-profile-under-throwaway-slug   ⛔ the incident
    //   isolated-but-not-by-slug               a fine bring-up, just not isolated
    //                                            BY the slug
    //
    // A consumer branching on "is there a warning?" would refuse a perfectly
    // safe configuration, and the only alternative would be matching the prose
    // — which silently weakens the moment the wording changes. Same defect as
    // `TEARDOWN_CONTRACT.keeps` shipping as prose, one field over, found by
    // applying that lesson here rather than waiting for it to be reported.
    // ⭐ SEVERITY IS BASE'S TO ASSIGN, BECAUSE THE VOCABULARY IS BASE'S TO GROW.
    // `kind` says WHAT; without `severity` nothing says HOW BAD, so every
    // consumer independently hardcodes which kinds are refusals — and they
    // drift, which is the copy-rot mechanism this family has hit five times.
    //
    // ⛔ The failure it prevents is specific: a consumer branching on a single
    // kind string silently ALLOWS a third dangerous kind added later. Branching
    // on `severity` handles new kinds correctly the day they ship, without the
    // consumer changing anything.
    //
    // ⚠ A consumer should still refuse an UNKNOWN severity rather than assume
    // it is benign — see PROFILE_WARNING_SEVERITIES. The asymmetry is the
    // argument: a new benign value blocking a bring-up is an annoyance that
    // prints what to do; a new dangerous value silently allowed is a
    // throwaway-named container mounting an authenticated profile.
    /** @type {{kind: string, severity: string, text: string}|null} */
    let warning = null;
    if (!slugIsDefault && isDefaultProfile) {
      warning = {
        kind: 'default-profile-under-throwaway-slug',
        severity: 'danger',
        text: `⛔ slug '${slug}' looks like a throwaway, but the profile resolves to `
          + `the DEFAULT profile (${resolved}). The containers will be named for the `
          + 'slug while mounting the default — often authenticated — profile. This is '
          + 'the combination that requires a lock to stop it.',
      };
    } else if (explicit && !slugIsDefault) {
      warning = {
        kind: 'isolated-but-not-by-slug',
        severity: 'notice',
        text: `slug '${slug}' is not 'default', but userDataDir is set explicitly, so `
          + 'the slug renames the CONTAINERS and does NOT move the PROFILE. The profile '
          + 'is isolated from the default, but not BY the slug.',
      };
    }

    return {
      path: resolved,
      source: explicit ? 'explicit-userDataDir' : 'derived-from-slug',
      isolatedBySlug,
      isDefaultProfile,
      slugIsDefault,
      warning,
