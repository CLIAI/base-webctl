...
// xpra-access operator CLI over the loopback REST (gateway P3): createXpraAccessCli(C, opts).
import * as xpraAccessCli from './browser-location/xpra-access-cli.js';
// Shared process/port mutex (sm2t): createProcessMutex(C, opts). p06y, expands v8m2.
import * as processMutex from './process-mutex.js';
// CDP command client: session/transport + target discovery. Extracted from
// substack-webctl (transport, production-proven) with the extension lane's
// discovery rewrite. Deliberately WITHOUT the observer-only guard, the
// credential deny-list, or per-axis target vocabulary — see the file header.
import * as cdpClient from './cdp-client.js';
// Storage-path resolution (v59v): createStoragePaths(C, opts). The single place
// a tool resolves cache/config/state/locks; XDG-aware, reads legacy locations,
// and deliberately ships no verb that can delete or move anything.
import * as storagePaths from './storage-paths.js';
// Counterparty-URL marking: detectors + the two record projections. Field
// selection is an explicit REQUIRED argument, never a default whole-object walk
// (sm2t "a seam classification is a claim"). Contributed by the chatgpt lane.
import * as urlMarking from './url-marking.js';
// Log rotation (sm2t): createLogRotation(C) with C.LOG_FILENAME_RE. Decides
// which log files rotation may DELETE — a pure predicate, no fs. The pattern is
// injected because consumers use different filename formats, and shipping one
// repo's pattern into another makes that repo's history unprunable. Refuses to
// construct without an anchored pattern, and refuses a malformed `keep` rather
// than defaulting to "delete everything". Contributed by the chatgpt lane.
import * as logRotation from './log-rotation.js';
