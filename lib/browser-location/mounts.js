...
export function createMounts(C, opts = {}) {
   ...
   *            isolatedBySlug: boolean, slugIsDefault: boolean, warning: string|null}}
   */
  function describeProfileResolution(slug, userDataDir) {
    ...
    return {
      path: profilePathFor(slug, userDataDir || null),
      source: explicit ? 'explicit-userDataDir' : 'derived-from-slug',
      isolatedBySlug,
      slugIsDefault,
      warning: (explicit && !slugIsDefault)
        ? `slug '${slug}' is not 'default', but userDataDir is set explicitly, so `
          + 'the slug renames the CONTAINERS and does NOT move the PROFILE. This '
          + 'bring-up will use the configured profile, not an isolated one.'
        : null,
    };
  }
