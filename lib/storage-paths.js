...
export function createStoragePaths(C, opts = {}) {
  ...
  const configRoot = path.join(xdgRoot(env, 'XDG_CONFIG_HOME', ['.config']), NAMESPACE, tool);
  const cacheRoot  = path.join(xdgRoot(env, 'XDG_CACHE_HOME',  ['.cache']),  NAMESPACE, tool);
  const stateRoot  = path.join(xdgRoot(env, 'XDG_STATE_HOME',  ['.local', 'state']), NAMESPACE, tool);
  ...
  const legacyCacheRoots = [
    path.join(xdgRoot(env, 'XDG_CACHE_HOME', ['.cache']), NAMESPACE, ...LEGACY_SHARED_SEGMENTS),
  ];
